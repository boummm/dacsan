Hotel.create!(name: "Cốm", address: "Hà Nội")
Hotel.create!(name: "Bánh đậu xanh", address: "Hải Dương")

20.times do |i|
  Hotel.create!(name: Faker::Food.dish, address: Faker::Address.city)
  HotelPicture.create!(hotel_id: i+1, picture: '000')
end
HotelPicture.create!(hotel_id: 21, picture: '000')
HotelPicture.create!(hotel_id: 22, picture: '000')
User.create!(name:  "boummm",
               email: "boummm1998@gmail.com",
               password: "thien1998",
               password_confirmation: "thien1998",
               picture: "avatar/#{rand(7)}.jpg"
              )
99.times do |n|
  name  = Faker::Name.name
  email = "user#{n + 1}@hotel.com"
  password = "123456"
  User.create!(name:  name,
               email: email,
               password: password,
               password_confirmation: password,
               picture: "avatar/#{rand(7)}.jpg"
              )
end
users = User.order(:created_at).take(6)
User.first.update_attributes(admin: true)
Admin.create!(email: "admin@gmail.com",
              password: "adminadmin")

HotelPicture.all.each do |ii|
  ii.update_column(:picture, "000")
end
